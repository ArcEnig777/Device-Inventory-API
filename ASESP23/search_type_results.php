...
if(isset($_POST['submit']) && ($_POST['submit']=="submit_t_m"))
{
	...
	else if ($query[0] == "All")
	{
		...
		inner join manufacturer
		on (manufacturer.m_auto_id = `manufacturer`)
		where manufacturer.m_name='$query2'";
	}
	else
	{
		$sql="Select COUNT(`s_auto_id`) 
	...
	else if ($query2 == "All")
	{
		...
		inner join manufacturer
		on (manufacturer.m_auto_id = `manufacturer`)
		where type.t_auto_id ='$query[0]' LIMIT $page_first_result,$results_per_page";
	}
	else if ($query[0] == "All")
	{
		$sql="Select `s_auto_id`, manufacturer.m_name AS manufacturer,`serial_num` 
		...
		inner join manufacturer
		on (manufacturer.m_auto_id = `manufacturer`)
		where manufacturer.m_name='$query2' LIMIT $page_first_result,$results_per_page";
	}
	else
	{
		$sql="Select `s_auto_id`, manufacturer.m_name AS manufacturer,`serial_num` 
		...
		inner join manufacturer
		on (manufacturer.m_auto_id = `manufacturer`)
		where type.t_auto_id ='$query[0]' and manufacturer.m_name='$query2' LIMIT $page_first_result,$results_per_page";
	}
	...
	echo "<input type=hidden name=type_manufacturer value='$query2'>";
	echo '<input type=hidden name="submit" value="submit_t_m">';
	if($page > 1)
	{
